...
function makeWord(lettersObject) {
  const wordArr = Object.entries(lettersObject).reduce(
    (result, [letter, positions]) => {
      return positions.reduce((acc, index) => {
        const newResult = [...acc];
...
function sellTickets(queue) {
  const price = 25;
  let total = 0;
  let avaliableMoney = true;

  queue.forEach((customerMoney) => {
    if (customerMoney > price && customerMoney - price > total) {
      avaliableMoney = false;
    }
    total += price;
  });
...
function sortCitiesArray(arr) {
  ...
  const sortedObjArr = [];
  arr.forEach((e) => {
    sortedArr.push(Object.entries(e));
  });

  sortedArr.sort().forEach((e) => {
    const value1 = e[0][1];
    const value2 = e[1][1];
    const newObj = {
      country: value1,
...
class CssSelector {
  ...
  checkOrder(order) {
    ...
    const orderIndexes = this.order.map((o) => correctOrder.indexOf(o));
    if (!orderIndexes.every((val, i, arr) => i === 0 || val >= arr[i - 1])) {
      throw new Error(
